...
function TimePresetPicker({
  ...
  const today = startOfDay(parseDateKey(todayDateKey));
  ...
  const [appliedRangeStart, setAppliedRangeStart] = useState(today);
  const [appliedRangeEnd, setAppliedRangeEnd] = useState(today);
  const [draftRangeStart, setDraftRangeStart] = useState(today);
  const [draftRangeEnd, setDraftRangeEnd] = useState(today);
  ...
  const handleDateSelect = (date: Date) => {
    ...
    if (disablePastDates && isBeforeToday(nextDate, today)) {
      return;
    }
              ...
              <div key={`${monthDate.getFullYear()}-${monthDate.getMonth()}`} className="product-date-calendar">
                ...
                <div className="product-date-grid">
                  {cells.map((cell) => {
                    ...
                    const isDisabled = disablePastDates && isBeforeToday(cell.date, today);
  ...
  return (
    <aside className="card product-filter-card">
      ...
      <div className="product-filter-group">
        ...
        <TimePresetPicker label="All time" todayDateKey={todayDateKey} disablePastDates />
      </div>

      <div className="product-filter-group">
        <h3 className="product-filter-title">Created time</h3>
        <TimePresetPicker label="All time" todayDateKey={todayDateKey} />
      </div>
